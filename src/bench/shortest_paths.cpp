...
const size_t NEDGES = 2766607;
constexpr int DEFAULT_NNODES     = 1965206;
               ...
               const double p)
{
    ...
    std::uniform_int_distribution<size_t> rnd_st(1, 1000000000);//std::numeric_limits<int>::max());

    std::vector<edge_t> *edges = new std::vector<edge_t>[n];

    std::ifstream myfile;
    myfile.open("roadNet-CA.txt");          
    assert(myfile);//while (!myfile) fprintf(stdout, ":(\n");
...
bench_thread(T *pq,
             const int thread_id,
             vertex_t *graph, const int seed, std::promise<size_t> &&rlx)
{
      ...
      const struct settings &settings)
{
    ...
    for (int i = 0; i < settings.num_threads; i++) {
      std::promise<size_t> prms;
      futures.push_back(prms.get_future());
      threads[i] = std::thread(bench_thread<T>, pq, i, graph, settings.seed, std::move(prms));
    }
    ...
    size_t TotalRelaxations=0;
    for (auto &future : futures) //int i=0;i<settings.num_threads;i++)
      {
    	TotalRelaxations+=future.get();
      }
    size_t NodesComponent=0;
    const double elapsed = timediff_in_s(start, end);
    //fprintf(stdout, "%f\n", elapsed);
    for (size_t i = 0; i < settings.num_nodes; i++) {
      const vertex_t *v = &graph[i];
      const size_t v_dist = v->distance.load(std::memory_order_relaxed);
      if (v_dist < std::numeric_limits<size_t>::max()) NodesComponent++;
     }
    fprintf(stdout, "%f\n", 1.0*(0+NodesComponent));
